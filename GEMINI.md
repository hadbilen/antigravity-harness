...
10. **Failure Log (`MISTAKES.md`):** Upon encountering an unexpected failure, broken contract, or user correction, prepend an entry to `MISTAKES.md` at project root. If `MISTAKES.md` does not yet exist at the project root, initialize it from the empty template `~/.gemini/config/templates/MISTAKES.template.md` (never copy another project's incident log) before writing the entry. Each entry must contain these four fields:
...
15. **Upstream and Platform Collision Invariant:** When upstream Google Antigravity updates introduce new built-in skills, runtime system prompts, or tool directives that conflict with this constitution or local custom skills, do not silently pick a winner: report an explicit `[Upstream Directive Collision]` diagnostic naming both directives and ask the user to decide. Platform safety, permission and tool-use instructions are never treated as overridable by local rules. Until the user decides, follow the stricter of the two directives; test softening or rule relaxation remains prohibited.
...
17. **Harness Self-Audit & Meta-Consistency Invariant (`meta-auditor`):** When modifying or authoring files within the harness governance surface itself (`GEMINI.md`, `DESIGN.md`, `skills/`, `agents/`, `guard/`, `porter/`), the agent is strictly prohibited from declaring completion without executing the deterministic self-audit gate (`python3 scripts/meta_audit.py --strict`). All YAML frontmatter schemas must be valid, all cross-referenced subagents and templates must exist on disk, mutual exclusion barriers between diverging skill modes must be preserved, and Porter export parity must yield zero loss. Any detected critical inconsistency blocks delivery (`BLOCKED: Harness Meta-Consistency Violation`).
